...
```jsonc
{
  ...
  "is_airlock": false,                       // bool, 이 Room 이 에어록(전실)인지 — true 면 airlocks[] 에 동일 실체 존재
  "airlock_id": null                         // string | null, is_airlock=true 일 때 대응 AirLock.al_id, 그 외 null
}
```

> **에어록 중복(dedup) 안내 (Doc Agent #3, 2026-05-31)**
> URS Room 시트에 전실이 방으로 입력돼 있어, 전실 18개가 `rooms[]`(Room)와
> `airlocks[]`(AirLock) 양쪽에 존재한다. **`airlocks[]` 가 전실의 단일 소스**다.
> 기본 출력에서 `rooms[]` 의 전실 항목은 `is_airlock=true` / `airlock_id` 로 표시만
> 되며 그대로 남는다(기존 계약 유지). 중복을 제거하려면 두 방법 중 하나:
> (a) 소비자 측에서 `is_airlock=true` Room 을 걸러낸다, 또는
> (b) 엔진 입력 `overrides.exclude_airlock_rooms=true` 로 호출하면 `rooms[]` 와
> `zones` 에서 전실 Room 이 제거된다(이때 `rooms` 48→30). `airlocks[]` 와
> `adjacency` 는 어느 경우에도 영향 없다.
...
```jsonc
{
  ...
  "area_m2": 9.0,                            // float, KB 권장 footprint: MAL 12.0 / PAL·CAL 9.0 m² (#1, kind별 차등, URS 면적 있으면 우선)
...
from rule_engine.urs_parser import load_urs_as_input
from rule_engine.models import Overrides

# 1) URS xlsx → RuleEngineInput → 7블록 출력 (기본: 에어록은 rooms[] 에
#    placeholder 로 남고 is_airlock=true 로 표시됨).
output = run_rule_engine(load_urs_as_input())
data = output.to_dict()              # dict (JSON 직렬화 가능)
json_str = output.to_json(indent=2)  # 문자열로 바로 저장 가능

# 2) 에어록 중복을 출력 단계에서 제거하고 싶으면 (rooms 48→30):
output_dedup = run_rule_engine(
    load_urs_as_input(overrides=Overrides(exclude_airlock_rooms=True))
)

# 3) 소비자 측에서 직접 거르는 방법 (기본 출력 사용 시):
real_rooms = [r for r in data["rooms"] if not r["is_airlock"]]
# airlock_rooms[i]["airlock_id"] ↔ data["airlocks"][j]["al_id"] 로 매칭.
```
