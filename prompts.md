...
## 2026-06-07 (추가 지시) — 안정화 후 UI + 3변형 산출 + 제약위반 수정

전수 감사(D-035 마스터목록 docs/audit_0607_worklist.md) 승인. W1부터 의존순 진행.
reconciliation 확정: D배치 유지+면적비례만 수정 / 도어 통일 유지 / 공정실끼리 도어 없음.

**추가 요구 (급하지 않으나 전부 완수)**:
1. **UI** — 동선 종류별 on/off 토글 보기 + 원하는 종류만 포함 다운로드.
   (현재 SVG 에 종류별 `<g id="flow-{key}">` 그룹 존재 → 토대 있음. UI/내보내기 확장.)
2. **한 URS → 배치 3개 산출**(선택 가능). 단 3개 모두 룰·제약 만족 필수.
   (다양성 E14/E15 를 "유효 변형 N개 제시"로 제품화.)
3. **rule_engine/RAG validation 제약이 layout 에 위반되는 것 원인분석+수정.**
   특히 "파서는 읽는데 블럭(building 등) 안 넘김"(=W1) + validation/rule_engine 쪽 수정 포함.
