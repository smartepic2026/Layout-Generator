...
> **Drawing Agent 세부 모듈과 Coordinate Planner 설명**은 → [docs/DRAWING_AGENT_MODULE_ARCHITECTURE.md](docs/DRAWING_AGENT_MODULE_ARCHITECTURE.md)
...
- **[2026-06-06 / (이 커밋)]** **Phase 1 (D-023) — 동선 화살표 flow_paths 기반 재작성 + 방 누락 해소.** ① `renderer._emit_z9_flow_arrows` 가 이제 `spec` 을 받아 `spec.flow_paths` 4종(personnel 입/퇴·material·waste·product)을 실제 좌표로 연결(종류별 색+수직 오프셋+구간 화살표, 외부 노드→URS 방위 외곽 포트, product=벽 가로지르기). placeholder spec 은 토폴로지 휴리스틱 폴백(`_topology` 보존). ② **방 누락 버그 해소**: `cli draw` 가 기본 strip-band 로 호출해 새 엔진 방(MEDIA/BUFFER 공정실 포함 7개)을 드롭하던 문제 → cli draw 기본을 gradient(전 방 배치, 누락 0)로 변경 +`--strip` 레거시 +누락 WARN. strip-band 코드 무수정(baselines 경계). 렌더=gradient(full)/측정=strip(subset) 역할 분리. 테스트 `test_drawing_agent` 갱신+신규 2건(누락 가드·flow 검증) 7건 통과, 전체 17→13 fail/100→106 pass(회귀 0). 산출 `output/bench_v3_flowpaths.svg`.
- **[2026-06-06 / (이 커밋)]** **Phase 2 (D-024) — 면적 비례 treemap + is_airlock dedup.** ① **G2 면적 왜곡(피드백 #5)**: gradient 솔버 그려진/spec 배율 0.54~5.84(왜곡, Cell bank 20㎡≡Material 100㎡). 원인=고정폭 컬럼+`_alloc_dim` 종횡비 클램프. **`_squarify`/`_place_treemap` 신규(squarified treemap, Bruls 2000)** → NC·Grade D 컬럼 면적 비례 배치. 결과 Cell bank 23㎡/Material 116㎡(둘 다 1.16×, 5배차 올바르게 비례). 공정행은 supply↔return 단일행 one-way 토폴로지 유지 위해 미적용(trade-off). ② **G3 is_airlock dedup**: 내부 Room 스키마에 is_airlock/airlock_id 추가(18→0 드롭이 18/18 흡수), `_is_al_fake_room` 1차 신호로 사용(패턴 폴백)—엔진이 전실에 면적 줘도 견고. drawing+adapter 47 통과. 산출 `output/bench_v4_treemap.svg`.
- **[2026-06-06 / (이 커밋)]** **Phase 1.5 (D-025) — 동선 화살표 직교(Manhattan) 라우팅.** D-023 동선이 방 중심 직선(대각선)이라 긴 대각선이 도면 가로질러 시인성 저하 → `_draw_flow_polyline` 를 L자 직교(수평→수직 엘보, 종류별 오프셋 유지)로 변경. 화살표는 노드 도착 다리에만, 직선 구간 단일 세그먼트(화살표 누락 버그 수정). 대각선 클러터 제거 → 복도 따라 흐르는 GMP 도면 가독성. drawing 7건 통과. 산출 `output/bench_v5_ortho.svg`.
- **[2026-06-06 / (이 커밋)]** **Phase 1.6 (D-026) — 동선 복도 인지 라우팅.** 사용자 지적(동선이 NC 방·벽 관통). flow_paths 복도 웨이포인트를 중심점 대신 복도 중심선상 [진입,진출]로 펼쳐 동선이 복도를 타고 흐르게(`_corridor_axis_points`/`_is_corridor_room`). Product 는 공정실 간 벽 가로지르기 정상(규정 3-2) 유지. drawing 7건 통과. 산출 `output/bench_v6_corridor.svg`. 한계: flow_paths 가 건너뛴 교차구역 점프는 Phase 1.7 완전확장에서.
- **[2026-06-06 / (이 커밋)]** **Phase 1.7a (D-027) — Product 동선 DS 보관실 연장.** 규정 3-1/3-2: product_process_order 끝에 Supply corridor + DS 보관실 연장. 공정실 간 벽 가로지르기 유지. drawing 7건 통과.
- **[2026-06-06 / (이 커밋)]** **Phase 1.7b (D-028) — 공정실별 comb 동선 + 토글.** 규정 4·2: one-way 공정실마다 Waste/Material 펼침(`_derive_full_flows`) → comb. 토글 `cli draw --flows {full|main|off}` + 종류별 `<g id="flow-{key}">` 그룹. drawing 7건 통과.
- **[2026-06-06 / (이 커밋)]** **Phase 1.8 (D-029) — 장애물 회피 채널 라우팅.** 사용자 재지적(동선 벽 관통). Product 외 동선을 채널(복도+외곽+방 경계선 그리드) 경유 직교 최소관통 경로로(`_channel_route`). 비-product 관통 main 10→8/full 25→15.
- **[2026-06-06 / (이 커밋)]** **Phase 3 #1 (D-030) — NC↔Grade D 순환 복도 신설.** 도면 피드백 #1. 분류기 `nc_corridor` + 솔버가 NC·D 사이 세로 복도 strip 카브(없으면 `_synth_corridor_room` 합성). 사람 순환+flow 채널 동시 확보 → 관통 main 8→6/full 15→13. 산출 `output/bench_v10_nccorr.svg`.
- **[2026-06-06 / (이 커밋)]** **Phase 3 #2~4 (D-031) — 도면 피드백 도어삭제·Waste/Material분리·접경게이트.** #2 주요 공정실끼리 도어 삭제(`_place_doors` 동일등급 process↔process 스킵, 공정물=벽/CIP 배관 이동) 도어 43→37. #3 Material-in 상단/Waste-out 하단 분리(94% of H, 교차오염 방지). #4 supply 밴드=[Gowning][MAL-in 반입실][supply복도] 접경 게이트(`_synth_gate_room`). drawing 7건 통과. 산출 `output/bench_v11_feedback.svg`.
- **[2026-06-06 / (이 커밋)]** **정렬 마무리 (D-032) — 미반영 룰엔진 필드 전수 소비.** 전수 재조사(alignment_audit §6) DROP 필드 반영: A 방 라벨에 ACPH·천정고·갱의(gowning_type). B 엔진 zones 1차 소비(CIP공급·모니터링 aux 불일치 해소). C airlock flow_type(cascade/sink/bubble) 표기 + door_count 분산배치. D constraints 소비(corridor_width 2000mm·equipment_clearance 1000 — eq_gap 800 GMP위반 수정, gradient 경로만 strip-band/baselines 불변). drawing 7건 통과. 산출 `output/bench_v12_allfields.svg`.
- **[2026-06-06 / (이 커밋)]** **새 15룰 엔진 모노레포 통합 (D-033).** 팀원 6/2 push(teammate/main `e05b85c`)의 새 엔진을 `src/rule_engine` 에 반영(완전 대체). 6/1 통합본 대비 6/2 차이 9파일(models is_airlock·engine·rule_06 airlock dedup·rule_13 차압·derive·output) 교체 — 신·구 모두 상대 import라 surgery 불필요. rag_validator 는 제외(rag_interface 절대 import 불일치, 파이프라인 불필요). 검증: `cli rule-engine` → **is_airlock=True 18**(새 엔진 증거), draw end-to-end 정상, 전체 13fail/106pass **회귀 0**. FINAL 도면 정식 파이프라인(URS→새엔진→draw)으로 재생성. 이제 cli rule-engine 이 새 15룰(is_airlock dedup·차압·ACPH·gowning) 호출 — 팀장님 URS 테스트가 새 룰로 동작.
- **[2026-06-06 / (이 커밋)]** **팀장님용 URS 입력 폴더 `input_urs/` 신설.** 본인 URS 엑셀을 넣는 전용 폴더 + 안내문(`README.md`) + 복사용 `예제_URS_템플릿.xlsx`. 템플릿으로 end-to-end(룰엔진 48방/18에어록 → draw 32방 배치) 동작 확인. `docs/USAGE.md` 갱신: 입력 경로를 `examples/` → `input_urs/` 로 안내, **§1-3 "나중에 업데이트 받기(git pull)" 섹션 추가**(클론은 스냅샷이라 갱신분은 pull 필요 + 충돌 주의 안내).
- **[2026-06-07 / (이 커밋)]** **0607 전수 감사(워크플로우) + W1 건물 footprint 계약 포함 (D-035).** ultracode 5에이전트 감사→18항목 마스터목록(`docs/audit_0607_worklist.md`). W1: 계약 `RuleEngineOutput.building` 필드 추가 + `cmd_rule_engine` URS building 매핑 + `cmd_draw` spec.building 캔버스 사용 — "파서는 읽는데 계약에 안 넘기던" 캔버스/면적 미반영 근본 해결(일반화 검증·회귀 0). 추가지시 기록: UI 동선토글/선택다운로드·URS당 3변형·제약위반 수정.
- **[2026-06-07 / (이 커밋)]** **팀장님 0607 피드백 — 인접성/배치 확정 (D-034, 팀장님 승인).** 중앙스파인 시도→폐기(NC직결 오염위반), **지난번 구조 복원**. D구역 **명시 2D 배치**(`_place_d_zone_rows`): Gowning F/M 전폭묶음 + Monitoring|DS·CIP|IPC/Cellbank 좌우 + Material-in 상단·Waste-out 하단. **containment 도어**(D방→D복도만, Gowning M/F만 NC↔D 게이트, 지원실은 Grade D 아니라 앞방 경유). **D→C 게이트 Gowning/MAL-in 세로스택**. **도어 폭 통일**(`_normalize_door_widths`). drawing 7/7, 회귀 0. FINAL 재생성. ⚠ D배치 ID 하드코딩 → 다음 커밋서 방 속성기반 일반화 예정(다른 URS 대응).
